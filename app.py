...
Present mode = the 30-min deck (what happened). Drill-down adds supporting detail
...
import snapshots
from definitions import MARKET_ORDER, GTM_ENGINES, fmt_triple
...
def snap_delta(metric, current):
    """WoW % vs the most recent prior weekly snapshot (None if no history)."""
    d, prior = snapshots.latest_before(today)
...
if meeting == "TOF Review":
    ...
    for i, (m, lbl) in enumerate([("pipeline_arr", "Pipeline ARR"), ("pipeline_acv", "Pipeline ACV"),
        ...
        if att is not None:
            parts.append(f"{att:.0f}% plan")
        r2[i].metric(lbl, money(tot[m]), " · ".join(parts) or None)
    st.caption("Metric language: **value (WoW Δ, % to plan)** per the V2 spec. WoW activates once "
               "weekly snapshots accrue (capturing from today). SQL-Booked WoW is live now.")
    ...
    st.markdown("### 2. Market Performance")
    ...
    sb, sh = gtm["sql_booked"].where(gtm["sql_booked"] != 0), gtm["sql_held"].where(gtm["sql_held"] != 0)
    gtm["held_rate"] = (gtm["sql_held"] / sb * 100).round(0)
    gtm["sal_rate"] = (gtm["sal"] / sh * 100).round(0)
    c1, c2 = st.columns(2)
    with c1:
    ...
    # ---- Drill-down extras: raw funnel tables (the "why") ----
...
# ===================== BOOKING REVIEW =====================
    ...
    # ---- 1. Executive Forecast (Week / Month / Quarter) ----
    st.markdown("### 1. Executive Forecast")
    ...
    # Coverage ratio + forecast hygiene (V2: 'will we hit the number?' + forecast discipline)
    k = st.columns(3)
    k[0].metric("Pipeline coverage", "—" if coverage is None else f"{coverage:.1f}×",
                cov_word, delta_color="off")
    ...
    # ---- 2. Forecast Movement ----
    st.markdown("### 2. Forecast Movement (Week-over-Week)")
    pd_date, prior = snapshots.latest_before(today)
    mv = []
    # Bookings ARR (QTD) is reconstructable from closedate history → live movement now.
    ...
    # Commit / Best / Weighted are point-in-time (forecast category has no history) → accrue forward.
    ...
    # ---- 3. Market Forecast View ----
    st.markdown("### 3. Market Forecast View")
    ...
    # ---- 4. Product Forecast View ----
    st.markdown("### 4. Product Forecast View")
    c1, c2 = st.columns(2)
    with c1:
    ...
    # ---- 6. Product × Market View ----
    st.markdown("### 6. Product × Market View")
    ...
    # ---- 8. Pod Reviews (V2 format) ----
    st.markdown("### 8. Pod Reviews")
    ...
    has_prior = snapshots.latest_before(today)[1] is not None
    cols = st.columns(2)
    ...
    # ---- 8b. Deal Watchlist (supporting / forecast-spec) ----
    st.markdown("#### Deal Watchlist")
    ...
    # ---- Drill-down extras: Wins + full pipeline ----
    if mode.startswith("Drill"):
        st.divider()
        st.markdown("### Wins")
